...
def convolutional_block_transpose(X_input, kernel_size, in_filter,
                                  out_filters, stage, block, stride=2):
    """
    Implementation of the convolutional block as defined in Figure 4
...
# stage 2
x2, w_conv = convolutional_block_transpose(X_input=x1, kernel_size=3, in_filter=64, out_filters=[64, 64, 256], stage=2,
                                           block='a',
                                           stride=1)
...
wconv, loss = sess.run([w_conv, cross_entropy], feed_dict={
    x: mnist.test.images,
    y: mnist.test.labels,
    keep_prob: 1.0})
