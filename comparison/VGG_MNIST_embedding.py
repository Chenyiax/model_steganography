...
with tf.name_scope('conv4_1') as scope:
    ...
    x_random1 = tf.cast(x_random1, tf.float32)
...
with tf.Session() as sess:
    ...
    for step in range(num_steps):
        ...
        if step % 100 == 0:
            ...
            print("\nstep: {:.1f} training accuracy: {:.4f} extraction error: {:.4f}".format(step, train_accuracy,
    ...
    accuracy_test = accuracy.eval(
        feed_dict={X: mnist.test.images,
                   ...
                   z1: watermarking1,
                   keep_prob: 1.0})
    wconv, loss_op = sess.run([w_conv, loss], feed_dict={
        X: mnist.test.images,
        y: mnist.test.labels,
        z1: watermarking1,
        keep_prob: 1.0})
...
np.save('vgg_weight_with_secret23.npy', wconv)
